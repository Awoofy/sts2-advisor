    ...
    <div className="bg-spire-panel border border-spire-border rounded-lg p-4">
      ...
      {state.current_position && (
        <div className="text-sm text-spire-muted mb-3">
          Current: {state.current_position.type} ({state.current_position.col},{' '}
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold">
                    {nodeTypeLabels[opt.type] ?? opt.type}
                  </span>
                  {isRecommended && (
                    <span className="text-xs bg-spire-green/20 text-spire-green px-2 py-0.5 rounded">
                      Recommended
                    </span>
                  )}
                </div>
